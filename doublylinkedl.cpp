...
int main()
{
    ...
    cin >> n;
    newNode = deleteNode(newNode, n);
    printLinkedlist(newNode);

    /*  cout<<"Removing Duplicate Node from Doublylinkedlist***************"<<endl;

      newNode=removeDuplicates(newNode);
      printLinkedlist(newNode);*/

    cout << "Finding the largest Node and Deleting it**************" << endl;

    newNode = largestNodeAndDelete(newNode);
    printLinkedlist(newNode);

    int N, M;
    cout << "Enter N and M for rotating Linkedlist counter-clockwise by N nodes and then clockwise by M nodes****" << endl;
    cin >> N >> M;

    newNode = rotate(newNode, N, M);
    printLinkedlist(newNode);
